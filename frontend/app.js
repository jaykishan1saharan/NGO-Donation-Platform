...
(function () {
  ...
  class DonationPlatform {
    ...
    loadUsersTable() {
      ...
      container.innerHTML = `<table class="data-table"><thead><tr><th>Name</th><th>Email</th><th>Type</th><th>Join Date</th><th>Total Donated</th></tr></thead><tbody>${
        this.users.map(u => `<tr><td>${u.name}</td><td>${u.email}</td><td>${u.type || ''}</td><td>${u.joinDate || ''}</td><td>₹${(u.totalDonated||0).toLocaleString()}</td></tr>`).join('')
      }</tbody></table>`;
    }
    ...
    loadDonationsTable() {
      ...
      container.innerHTML = `<table class="data-table"><thead><tr><th>Date</th><th>Donor</th><th>Amount</th><th>Method</th><th>Status</th></tr></thead><tbody>${
        this.donations.map(d => {
    ...
    loadNGOsTable() {
      ...
      container.innerHTML = `<table class="data-table"><thead><tr><th>Name</th><th>Location</th><th>Verified</th><th>Beneficiaries</th><th>Established</th></tr></thead><tbody>${
        this.ngos.map(n => `<tr><td>${n.name}</td><td>${n.location}</td><td>${n.verified ? 'Verified':'Pending'}</td><td>${n.beneficiaries}</td><td>${n.established}</td></tr>`).join('')
      }</tbody></table>`;
    }
